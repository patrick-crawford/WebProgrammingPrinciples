---
id: intro
title: Table of contents
sidebar_position: 1
slug: /
description: Contents ofWeb Programming Principles
---

# Table of contents

## Introduction

- [Welcome](Introduction/Welcome)
- [Internet Architecture](Introduction/Internet-Architecture)
- [Introduction to JavaScript](Introduction/Introduction-JavaScript)
- [Practice Exercises](Introduction/Practice-Exercises)

## Functions & Scope

- [Functions](Functions-Scope/Functions)
- [Scope & Closures](Functions-Scope/Scope-Closures)
- [Practice Exercises](Functions-Scope/Practice-Exercises)

## Objects & Built-ins

- [Introduction](Objects-Built-ins/Introduction)
- [Strings](Objects-Built-ins/Strings)
- [Arrays](Objects-Built-ins/Arrays)
- [RegExp](Objects-Built-ins/RegExp)
- [Practice Exercises](Objects-Built-ins/Practice-Exercises)

## Advanced Objects & Inheritance

- [Objects in JavaScript](Advanced-Objects-Inheritance/Objects-JavaScript)
- [Constructor Functions](Advanced-Objects-Inheritance/Constructor-Functions)
- [Practice Exercise](Advanced-Objects-Inheritance/Practice-Exercise)

## HTML Fundamentals

- [Development Web Environment](HTML-Fundamentals/Development-Web-Environment)
- [HTML](HTML-Fundamentals/HTML)
- [Practical Examples](HTML-Fundamentals/Practical-Examples)

## Elements & Media

- [HTML Elements](Elements-Media/HTML-Elements)
- [Multimedia](Elements-Media/Multimedia)
- [Practical Examples](Elements-Media/Practical-Examples)

## DOM & Events

- [DOM Introduction](DOM-Events/DOM-Introduction)
- [Events & Timers](DOM-Events/Events-Timers)
- [Practice Exercise](DOM-Events/Practice-Exercise)

## CSS & Styling

- [Introduction to CSS & Syntax](CSS-Styling/Introduction-CSS-Syntax)
- [Applied CSS](CSS-Styling/Applied-CSS)

## Box Model & Layout

- [Box Model](Box-Model-Layout/Box-Model)
- [Practice Exercise](Box-Model-Layout/Practice-Exercise)

## Forms & Inputs

- [HTML Forms Introduction](Forms-Inputs/HTML-Forms-Introduction)
- [Forms & CSS](Forms-Inputs/Forms-CSS)
- [Practice Exercise](Forms-Inputs/Practice-Exercise)

## Client-side Validation

- [HTML5 Form Validation](Client-side-Validation/HTML5-Form-Validation)
- [Using JavaScript](Client-side-Validation/Using-JavaScript)

## JSON & Dynamic Data

- [AJAX Fundamentals](JSON-Dynamic-Data/AJAX-Fundamentals)
- [Data Formats](JSON-Dynamic-Data/Data-Formats)
- [Advanced Techniques](JSON-Dynamic-Data/Advanced-Techniques)

## Case Study

- [Using REST APIs to Integrate Data in Web Apps](Case-Study/Using-REST-APIs-Integrate-Data-Web-Apps)

## Resources

- [Learning Resources](Resources/learning-resources)
- [Static Web Site Hosting](Resources/Static-Web-Site-Hosting)
